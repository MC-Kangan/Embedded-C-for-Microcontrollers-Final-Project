...
void stop(struct DC_motor *mL, struct DC_motor *mR)
{
    ...
    __delay_ms(500);    // buggy will stop there for 0.5 seconds after the motor power becomes 0
}
...
void fullSpeedAhead(struct DC_motor *mL, struct DC_motor *mR)
{
    ...
    while (mL->power<FORWARD_POWER_L || mR->power<FORWARD_POWER_R){     // when the powers for left and right motors are smaller than the setting value(see dc_motor.h)
        ...
        if (mR->power<FORWARD_POWER_R) {mR->power += 2;}                // increase the right motor power by 2 for each time (this case the power of motor doesn't push to the setting value immediately)                                             
        setMotorPWM(mL);                                                // set the power to motor
        setMotorPWM(mR);                                                // set the power to motor
        __delay_ms(10);
    }
...
void fullSpeedBack(struct DC_motor *mL, struct DC_motor *mR, unsigned char instruction)
{
    ...
    while (mL->power<BACKWARD_POWER || mR->power<BACKWARD_POWER){   // when the powers for left and right motors are smaller than the setting value(see dc_motor.h)
        if (mL->power<BACKWARD_POWER) {mL->power += 2;}             // increase the left motor power by 2 for each time (this case the power of motor doesn't push to the setting value immediately)
        if (mR->power<BACKWARD_POWER) {mR->power += 2;}             // increase the right motor power by 2 for each time (this case the power of motor doesn't push to the setting value immediately)
        setMotorPWM(mL);                                            // set the power to motor
        setMotorPWM(mR);                                            // set the power to motor
        __delay_ms(5);
    }
